# Módulo 3: Montagem de Portfólio e Investimento

Analisar ativos um a um não basta: investir bem é combinar ativos numa carteira que sirva aos **seus** objetivos e que você consiga manter nos dias ruins. Este módulo ensina o processo completo — do perfil de investidor à rotina de aportes e rebalanceamento.

## Objetivo

Ao final do módulo, você deve conseguir montar e defender uma carteira completa: definir alocação por classe de ativos com base em perfil e objetivos, medir o risco do conjunto, escolher veículos eficientes em custo e estabelecer regras escritas de manutenção.

## Pré-requisito

Concluir o [Módulo 1: Fundamentos em Finanças](../modulo-1-fundamentos/index.md). O [Módulo 2: Análise Fundamentalista](../modulo-2-analise-fundamentalista/index.md) complementa (as posições de convicção da carteira vêm de lá), mas não é obrigatório para começar.

## Unidades

1. [O que é um portfólio (e por que não ter um ativo só)](aula-01-o-que-e-um-portfolio.md)
2. [Perfil de investidor e objetivos](aula-02-perfil-de-investidor-e-objetivos.md)
3. [Classes de ativos e correlação](aula-03-classes-de-ativos-e-correlacao.md)
4. [Alocação de ativos](aula-04-alocacao-de-ativos.md)
5. [Risco de carteira na prática](aula-05-risco-de-carteira.md)
6. [Aportes e rebalanceamento](aula-06-aportes-e-rebalanceamento.md)
7. [Custos e impostos na carteira](aula-07-custos-e-impostos.md)
8. [Montando sua primeira carteira — estudo de caso completo](aula-08-montando-sua-carteira.md)

## Estrutura de cada unidade

- **Comece aqui**: a ideia principal sem jargão.
- **Conceitos essenciais**: termos que você precisa dominar.
- **Exemplo resolvido**: aplicação passo a passo.
- **Mini quiz**: questões curtas de fixação.
- **PBL**: problema aberto para pensar como gestor da própria carteira.
- **Resumo**: o que levar para a próxima unidade.

## Próximo passo

Com os três módulos de Finanças concluídos, o caminho segue para a trilha de [Preparação BRHSIC](../../preparacao-brhsic/index.md).
